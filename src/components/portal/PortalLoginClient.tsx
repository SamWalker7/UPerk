...
export function PortalLoginClient({ next }: { next?: string }) {
  ...
  // If a session already exists, bounce straight past the form — checked
  // client-side against our own /portal/api/session route (the cookie is
  // httpOnly, so the browser can't read it itself).
  ...
  useEffect(() => {
    ...
    fetch("/portal/api/session")
      ...
      .then((body) => {
        if (cancelled) return;
        if (body?.role) router.replace(safeNext);
        else setCheckingSession(false);
      })
      .catch(() => !cancelled && setCheckingSession(false));
    return () => {
      cancelled = true;
    };
